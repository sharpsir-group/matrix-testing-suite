#!/bin/bash
# Comprehensive Privilege Management Tests
# Tests all admin-privileges endpoints: grant, revoke, templates, audit
...
echo "# Privilege Management Comprehensive Tests - $(date)" > "$RESULTS_FILE"
echo "" >> "$RESULTS_FILE"
echo "## Test Coverage" >> "$RESULTS_FILE"
echo "" >> "$RESULTS_FILE"
echo "This test suite covers all privilege management features:" >> "$RESULTS_FILE"
echo "- List privileges (GET /admin-privileges)" >> "$RESULTS_FILE"
echo "- Grant privilege (POST /admin-privileges/grant)" >> "$RESULTS_FILE"
echo "- Revoke privilege (POST /admin-privileges/revoke)" >> "$RESULTS_FILE"
echo "- List privilege templates (GET /admin-privileges/templates)" >> "$RESULTS_FILE"
echo "- Create privilege template (POST /admin-privileges/templates)" >> "$RESULTS_FILE"
echo "- Get audit log (GET /admin-privileges/audit)" >> "$RESULTS_FILE"
echo "" >> "$RESULTS_FILE"
...
echo "=== Privilege Management Comprehensive Tests ==="
echo ""
...
TIMESTAMP=$(date +%s)
TEST_USER_EMAIL="privilege.test.${TIMESTAMP}@sharpsir.group"
...
USER_RESPONSE=$(curl -s -X POST "${SSO_SERVER_URL}/admin-users" \
  ...
  -d '{
    ...
    "user_metadata": {"full_name": "Privilege Test User"}
  }')
...
# Test 1: List Privileges
echo "Test 1: List Privileges..."
PRIVS_RESPONSE=$(curl -s -X GET "${SSO_SERVER_URL}/admin-privileges" \
  -H "Authorization: Bearer ${ADMIN_TOKEN}")

PRIVS_COUNT=$(echo "$PRIVS_RESPONSE" | jq 'if type=="array" then length else if .privileges then (.privileges | length) else 0 end end' 2>/dev/null || echo "0")
ERROR=$(echo "$PRIVS_RESPONSE" | jq -r '.error // empty' 2>/dev/null || echo "")

if [ "$PRIVS_COUNT" -ge 0 ] && [ -z "$ERROR" ]; then
  log_test "List Privileges" "PASS" "Retrieved $PRIVS_COUNT privileges"
else
  log_test "List Privileges" "FAIL" "Failed: $PRIVS_RESPONSE"
fi
...
if [ -n "$TEST_USER_ID" ] && [ "$TEST_USER_ID" != "null" ]; then
  ...
  if [ -n "$PRIV_ID" ] && [ "$PRIV_ID" != "null" ] && [ -z "$ERROR" ]; then
    log_test "Grant Privilege" "PASS" "Successfully granted app_access privilege"
...
echo "Test 4: List Privilege Templates..."
TEMPLATES_RESPONSE=$(curl -s -X GET "${SSO_SERVER_URL}/admin-privileges/templates" \
  -H "Authorization: Bearer ${ADMIN_TOKEN}")
...
if [ "$TEMPLATES_COUNT" -ge 0 ] && [ -z "$ERROR" ]; then
  ...
  log_test "List Privilege Templates" "SKIP" "Templates endpoint may not exist or failed: $TEMPLATES_RESPONSE"
fi
...
AUDIT_RESPONSE=$(curl -s -X GET "${SSO_SERVER_URL}/admin-privileges/audit" \
  -H "Authorization: Bearer ${ADMIN_TOKEN}")
...
echo ""
echo "=== Privilege Management Tests Complete ==="
